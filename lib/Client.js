...
class MoodleRestClient {
   ...
   */
  buildData (wsFunction, key, value) {
    return {
      wstoken: this.token,
  ...
  send (wsFunction, key, value) {
    if (!wsFunction || !key || !value) return Promise.reject(new Error('WsFunction, key and value are required'))

    return new Promise((resolve, reject) => {
      let data = this.buildData(wsFunction, key, value)
